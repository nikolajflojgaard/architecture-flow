...
export async function POST(
  ...
  try {
    ...
    return NextResponse.redirect(new URL(`/work-items/${id}?rendered=${state}`, request.url));
  } catch {
    return NextResponse.redirect(new URL(`/work-items/${id}?rendered=error`, request.url));
  }
}

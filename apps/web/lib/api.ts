...
export async function getWorkItems(
  ...
  try {
    const response = await fetch(
      `${baseUrl}/v1/work-items?${searchParams.toString()}`,
      {
        cache: "no-store",
      },
    );
...
export async function getAuditEvents(id: string): Promise<AuditEvent[]> {
  try {
    const response = await fetch(
      `${baseUrl}/v1/work-items/${id}/audit-events`,
      {
        cache: "no-store",
      },
    );
...
export async function getArtifacts(workItemId: string): Promise<Artifact[]> {
  try {
    const response = await fetch(
      `${baseUrl}/v1/work-items/${workItemId}/artifacts`,
      {
        cache: "no-store",
      },
    );
...
export async function getTasks(workItemId: string): Promise<WorkflowTask[]> {
  try {
    const response = await fetch(
      `${baseUrl}/v1/work-items/${workItemId}/tasks`,
      {
        cache: "no-store",
      },
    );

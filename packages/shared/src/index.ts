...
export const serviceTaskTopics = [
  "intake.classify",
  "artifact.render-pdf",
] as const;
...
const customerRules: Array<{
  name: string;
  matches: (text: string) => boolean;
  customer: string;
}> = [
  {
    name: "customer:tdc-net",
    matches: (text) => /\btdc\s*net\b|\btdc\b/.test(text),
    customer: "TDC NET",
  },
  {
    name: "customer:bpi",
    matches: (text) => /\bbpi\b/.test(text),
    customer: "TDC NET",
  },
  {
    name: "customer:yousee",
    matches: (text) => /\byousee\b/.test(text),
    customer: "YouSee",
  },
  {
    name: "customer:nuuday",
    matches: (text) => /\bnuuday\b/.test(text),
    customer: "Nuuday",
  },
];
...
const domainRules: Array<{
  ...
  {
    name: "domain:customer-platform",
    matches: (text) =>
      /customer|address|myview|crm|salesforce|account/.test(text),
    domain: "Customer platform",
  },
  {
    name: "domain:integration",
    matches: (text) =>
      /integration|api|yaml|openapi|swagger|json|xml/.test(text),
    domain: "Integration / API",
  },
  {
    name: "domain:billing",
    matches: (text) =>
      /billing|invoice|payment|financ|bogholder|fas/.test(text),
    domain: "Billing / Finance",
  },
  {
    name: "domain:network",
    matches: (text) => /network|fiber|access|infra|provision/.test(text),
    domain: "Network / Provisioning",
  },
];

export function inferIntakeMetadata(
  input: IntakeMetadataInput,
): IntakeMetadataInference {
  ...
  const isApiRelated =
    isYaml || isApiFolder || /\bapi\b|openapi|swagger/.test(haystack);
  ...
  if (
    !customer &&
    (normalizedFolder === "General designs" ||
      normalizedFolder === "API spec drop/YAML")
  ) {
    customer = "TDC NET";
